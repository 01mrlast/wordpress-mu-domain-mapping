...
function dm_handle_actions() {
	...
	if ( !empty( $_POST[ 'action' ] ) ) {
		$domain = $wpdb->prepare( $_POST[ 'domain' ] );
		if ( $domain == '' ) {
...
function domain_mapping_siteurl( $setting ) {
	...
	if ( !isset( $return_url[ $wpdb->blogid ] ) ) {
		...
		if ( get_site_option( 'dm_no_primary_domain' ) == 1 ) {
			$domain = $wpdb->get_var( "SELECT domain FROM {$wpdb->dmtable} WHERE blog_id = '{$wpdb->blogid}' AND domain = '" . $wpdb->prepare( $_SERVER[ 'HTTP_HOST' ] ) . "' LIMIT 1" );
			if ( null == $domain ) {
